...
if ( opts.visualize ) 
  if ( size(res.tstconf,2)==1 ) % confusion matrix is correct
    ...
    net_res = reshape(res.tstconf(:,1,res.opt.Ci),sqrt(size(res.tstconf,1)), []);
    disp('Sum of rows:');
    row_sum = sum(net_res, 2)
    P_res = zeros(size(net_res,1),size(net_res,2));
    for ind1 = 1:size(net_res,1)
        for ind2 = 1:size(net_res,2)
            P_res(ind1,ind2) = net_res(ind1,ind2)/row_sum(ind1);
        end
    end
    ...
    condPFig=figure(5); set(condPFig,'name','Conditional probabilities');	 
	 t = uitable(condPFig, 'Data', P_res, 'Position', [20 20 262 204]);
